...
def run(svg_file: Path,
    ...
    aucroc = pd.read_csv(aucroc_file,
                         index_col=[0, 1],
                         header=[0, 1]).loc[:, ("average", "full__average")]
...
if __name__ == "__main__":
    ...
    ct_file = Path("out/tgev-full-pool/fold/tgev/full/full__average.ct")
    table_file = Path("out/tgev-full-pool/table/tgev/full/mask-per-pos.csv")
