...
In support, of the thirteen structures predicted from the cluster 1 DMS reactivities, nine (69\%) contained the innermost stem -- which we call long stem 1 (LS1) -- compared to five of eleven (45\%) using the ensemble average and four of twenty (20\%) using no DMS reactivities (Supplementary Figure~\ref{sars2-clusters-fold}).
The middle stem of the FSE-arch -- long stem 2 (LS2) -- was predicted in eight structures (62\%) using the cluster 1 DMS reactivities, and zero using the ensemble average or no DMS reactivities.
Thus, we found that the DMS reactivities of the long-range cluster enabled predicting the long-range stems more consistently.

Our refined model based on the long-range cluster (Figure~\ref{lnas}) included not only the two inner stems of the FSE-arch -- LS1 and LS2 -- but also two stems (LS3 and LS4) that were not in the original FSE-arch model~\cite{Ziv2020}.
