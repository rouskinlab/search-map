...
The RNA is chemically probed separately with (+ASO) and without (no-ASO) the ASO, followed by mutational profiling and sequencing, e.g. using DMS-MaPseq~\cite{Zubradt2016} (Figure~\ref{strat}c).

SEISMIC-RNA can detect RNA--RNA interactions by comparing the +ASO and no-ASO mutational profiles.
...
Because the structures -- and therefore mutational profiles -- of R differ between the interaction-formed and -unformed states, the ensemble averages of R also differ between the +ASO and no-ASO conditions (Figure~\ref{strat}d, middle).
...
SEISMIC-RNA can also determine the mutational profiles of the states where the P--R interaction is formed and unformed -- even if their secondary structures are unknown.
Inferring mutational profiles for the interaction-formed/unformed states requires clustering the no-ASO ensemble into two clusters of RNA molecules (Figure~\ref{strat}e, top); SEISMIC-RNA uses an optimized version of the DREEM algorithm~\cite{Tomezsko2020} for clustering.
...
Therefore, a cluster that correlates well with the +ASO ensemble average (here, cluster 2) corresponds to the interaction-unformed state, while a cluster that correlates weakly (here, cluster 1) corresponds to the interaction-formed state (Figure~\ref{strat}e, bottom).
...
\subsection{Structure of the full TGEV genome in ST cells supports a long-range RNA--RNA interaction involving the FSE}
